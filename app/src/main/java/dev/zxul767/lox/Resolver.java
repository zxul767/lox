...
class Resolver implements Expr.Visitor<Void>, Stmt.Visitor<Void> {
  enum VarResolution { DECLARED, DEFINED }
  ...
  // all other "resolve" functions simply traverse the AST
  ...
  private void resolveFunction(Stmt.Function function) {
    ...
    for (Token param : function.params) {
      // this call to "declare" is superflous but we add it for clarity
      declare(param);
      define(param);
    }
  ...
  private void declare(Token name) {
    if (scopes.isEmpty())
      return;
    scopes.peek().put(name.lexeme, VarResolution.DECLARED);
  }
  ...
  public Void visitVarStmt(Stmt.Var stmt) {
    declare(stmt.name);
    // we declare the variable's existence but not its definition
